...
public class StrategyFactory {
  ...
  /**
   * Creates and returns a {@link GameStrategy} suitable for the given game type,
   * configured with the provided board and dice.
   *
   ...
   * {@code gameType} is unsupported or unknown.
   */
  public static GameStrategy createStrategy(GameType gameType, Board board, Dice dice) {
    ...
    switch (gameType) {
      case SNAKES_AND_LADDERS:
        return new SnakesAndLaddersStrategy(board, dice);
      default:
        LOGGER.severe("Unsupported or unknown game type provided to StrategyFactory: " + gameType);
        throw new IllegalArgumentException("Unsupported or unknown game type: " + gameType);
    }
  }
}

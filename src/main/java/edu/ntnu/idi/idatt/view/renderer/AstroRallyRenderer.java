...
public class AstroRallyRenderer implements BoardRenderer {
  ...
  /**
   * Immediately animates a token jumping from its current tile to the given target tile,
   * then updates the token’s user data and invokes the completion callback.
   *
   * @param playerTokenNode the token Node to animate; must have PlayerTokenData userData
   * @param targetTile the Tile to which the token will jump; must not be null
   * @param boardPane the Pane containing the board and tokens; must not be null
   * @param onAnimationComplete a callback Runnable to invoke when the jump animation finishes
   */
  @Override
  public void animateTokenDirectly(final Node playerTokenNode, final Tile targetTile,
      final Pane boardPane, final Runnable onAnimationComplete) {
    requireNonNull(playerTokenNode, "PlayerTokenNode cannot be null.");
    requireNonNull(targetTile, "TargetTile cannot be null.");

    if (!(playerTokenNode.getUserData() instanceof PlayerTokenData tokenData)) {
      LOGGER.log(SEVERE,
          "Cannot animate token directly: UserData not PlayerTokenData. UserData: {0}",
          playerTokenNode.getUserData() != null
              ? playerTokenNode.getUserData().getClass().getName() : "null");
    ...
    final Player player = tokenData.getPlayer();
    final int currentTileId = tokenData.getCurrentTileId();
    final int destinationTileId = targetTile.getTileId();

    LOGGER.log(INFO, "Animating {0} directly from tile {1} to {2}.",
        new Object[]{player.getName(), currentTileId, destinationTileId});

    final Point2D currentPositionCenter = this.tileCenterPositions.get(currentTileId);
    final Point2D destinationPositionCenter = this.tileCenterPositions.get(destinationTileId);

    if (destinationPositionCenter == null) {
      LOGGER.log(SEVERE,
          "Direct animation failed: No center for destination tile ID {0}."
              + " Token remains at {1}.",
    ...
    if (currentPositionCenter == null) {
      LOGGER.log(WARNING,
          "Direct animation: No center for current tile ID {0}."
              + " Committing to destination {1}.",
    ...
    final TranslateTransition jumpAnimation =
        buildJumpTransition(playerTokenNode, currentPositionCenter, destinationPositionCenter);
